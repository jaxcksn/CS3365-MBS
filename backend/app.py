...
load_dotenv()
app = Flask(__name__)
CORS(app, supports_credentials=True,resources={r"/*": {"origins": os.getenv("FRONTEND_URL", "https://mbs.jaxcksn.dev")}})
...
def admin_required(f):
    ...
    def wrap(*args, **kwargs):
        ...
        try:
            payload = jwt.decode(token, os.getenv("JWT_SECRET"), algorithms=['HS256'])
            user_id = payload.get('user_id')
            query = db.query("SELECT `role` FROM `User` WHERE id=:user", user=user_id)
...
def refresh_user():
    ...
    try:
        ...
        access_token, access_token_expiry = generate_access_token(user_id)
        new_refresh_token = generate_refresh_token(user_id)
        
        # Return the new tokens in response, set refresh token in HttpOnly cookie
        response = jsonify({'access_token': access_token, 'expires': datetime.datetime.fromtimestamp(access_token_expiry)})
        response.set_cookie('refresh_token', new_refresh_token[0], expires=new_refresh_token[1], httponly=True, secure=True, max_age=7*24*60*60)
        return response, 200
    except jwt.ExpiredSignatureError:
        return jsonify({'error': 'Refresh token has expired'}), 401
    except jwt.InvalidTokenError:
